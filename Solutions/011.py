...
data = '08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 ' \
    ...
    for j in range(0, COLS):
        ...
        if i - LENGTH >= 0:
            ...
            if greatest_product < temp:
                ...
                direction = '↑'
        # Вверх и вправо
        if i - LENGTH >= 0 and j + LENGTH <= COLS - 1:
            temp = 1
            for k in range(0, LENGTH):
                temp *= table[i - k][j + k]
            if greatest_product < temp:
                ...
                direction = '↗'
        # Вниз и вправо
        if i + LENGTH <= ROWS - 1 and j + LENGTH <= COLS - 1:
            temp = 1
            for k in range(0, LENGTH):
                temp *= table[i + k][j + k]
            if greatest_product < temp:
                ...
                direction = '↘'
        # Вниз и влево
        if i + LENGTH <= ROWS - 1 and j - LENGTH >= 0:
            temp = 1
            for k in range(0, LENGTH):
                temp *= table[i + k][j - k]
            if greatest_product < temp:
...
print(f'Столбец: {col_index + 1}')
print(f'Наибольшее произведение: {greatest_product}')
print(f'Напрвление: {direction}')
